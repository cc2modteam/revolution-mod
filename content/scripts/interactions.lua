...
function on_sidebar_message(message)
	...
	local lines = get_text_lines(message)
	for i, line in pairs(lines) do
		if i > 1 then
			...
			elseif line == "clear" then
				...
				for ii, v in pairs(rx) do
					if ii == 1 then
						item.type = v
					end
					if item.type == "text" then
						if ii == 2 then
							item.x = tonumber(v, 10)
						elseif ii == 3 then
							item.y = tonumber(v, 10)
						elseif ii == 4 then
							item.txt = v
						elseif ii == 5 then
							item.color = sidebar_color(v)
						end
					end
				end
				if item.type then
...
function process_sidebar(msg)
	...
	if msg.message:sub(1, 1) == "!" then
		if msg.timestamp > sidebar.last then
			sidebar.last = msg.timestamp
			-- handle the message
			-- print(msg.timestamp, msg.message)
			on_sidebar_message(msg.message)
		end
		return true
	end
	return false
...
function render_sidebar(ui, screen_w, screen_h)
	local sidebar = g_sidebar

	if sidebar.open then
		update_ui_push_alpha(180)
		local w = math.floor(sidebar.w * screen_w)
...
function render_chat(screen_w, screen_h, delta_time)
	...
	if #messages > 0 then
		for i = 1, #messages do
			...
			local skip = process_sidebar(msg)
			if not skip then
				if msg.timestamp > g_chat_messages.last_rx then
					g_chat_messages.last_rx = msg.timestamp
					table.insert(g_chat_messages.messages, msg)
				end
			end
		end
		if #g_chat_messages.messages > 5 then
			-- trim off oldest
			local old = g_chat_messages.messages
			g_chat_messages.messages = {}
			for i = 2, #old do
				table.insert(g_chat_messages.messages, old[i])
			end
		end
	end

	messages = g_chat_messages.messages
	...
	render_sidebar(ui, screen_w, screen_h)
